...
import { BarChart3, TrendingUp, Users, Clock, Shield } from 'lucide-react';
...
export default function Analytics() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([analyticsData, _surveyData]) => {
  ...
  const statsCards = [
    ...
    { label: 'Avg Duration', value: 'N/A', icon: Clock, color: 'text-purple-600 bg-purple-50' },
    {
      label: 'Responses Today',
      value: analytics.responsesByDate?.slice(-1)[0]?.count.toString() ?? '0',
      icon: Users,
                ...
                <div key={d.date} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 bg-gray-100 rounded-full h-4 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(d.count / Math.max(...analytics.responsesByDate.map(x => x.count), 1)) * 100}%`,
                      }}
                    />
                  </div>
              ...
              {analytics.questionAnalytics.map(qa => (
                <div key={qa.questionId} className="border-b pb-4 last:border-b-0">
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-500">
                    ...
                    {(qa as any).averageRating && <span>Avg rating: {(qa as any).averageRating.toFixed(2)}</span>}
                  </div>
                        ...
                        <div key={key} className="flex items-center gap-2">
                          ...
                          <div className="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden">
                            <div
                              ...
                              style={{ width: `${(count / qa.totalAnswers) * 100}%` }}
                            />
                          </div>
