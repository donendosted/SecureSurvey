        ...
        <Card className="max-w-md mx-4 text-center">
          <CardContent className="p-8">
            ...
            <CardDescription className="mb-6">
              Your response has been encrypted and stored on the Midnight Network. Thank you for your participation!
            </CardDescription>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex gap-1">
                {survey.questions.map((_, i) => (
                  <div
                    key={i}
                    className={`h-2 w-8 rounded-full transition-colors ${i <= step ? 'bg-primary-500' : 'bg-gray-200'}`}
                  />
                ))}
              </div>
            </div>
            ...
            <div className="space-y-6" key={currentQuestion.id}>
              ...
              {renderQuestion(currentQuestion, answers[currentQuestion.id], val =>
...
function renderQuestion(question: Survey['questions'][0], value: unknown, onChange: (val: unknown) => void) {
        ...
        <div className="flex gap-2">
          {Array.from({ length: (question as any).maxRating ?? 5 }, (_, i) => i + 1).map(n => (
            <button
              key={n}
              onClick={() => onChange(n)}
              className={`h-12 w-12 rounded-full font-bold text-lg transition-colors ${value === n ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
            >
              {n}
            </button>
          ))}
    ...
    case 'scale':
      return (
        <div className="space-y-2">
          <input
            type="range"
            min={(question as any).minValue ?? 1}
            max={(question as any).maxValue ?? 10}
        ...
        <div className="flex gap-4">
          {['Yes', 'No'].map(opt => (
            <button
              ...
              className={`flex-1 p-4 border-2 rounded-lg font-medium transition-colors ${value === (opt === 'Yes') ? 'border-primary-600 bg-primary-50 text-primary-600' : 'border-gray-200 hover:border-gray-300'}`}
            >
              {opt}
            </button>
          ))}
